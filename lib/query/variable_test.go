...
func TestVariableScopes_Substitute(t *testing.T) {
	...
	for _, v := range variableScopesSubstituteTests {
		ctx := ContextForExecusion(context.Background(), v.Filter)
		result, err := list.Substitute(ctx, v.Expr)
		if err != nil {
			if len(v.Error) < 1 {
				t.Errorf("%s: unexpected error %q", v.Name, err)
			} else if err.Error() != v.Error {
...
func TestVariableMap_Declare(t *testing.T) {
	...
	for _, v := range variableMapDeclareTests {
		...
		ctx := ContextForExecusion(context.Background(), v.Filter)
		err := vars.Declare(ctx, v.Expr.(parser.VariableDeclaration))
		if err != nil {
			if len(v.Error) < 1 {
				t.Errorf("%s: unexpected error %q", v.Name, err)
			} else if err.Error() != v.Error {
...
func TestVariableMap_Substitute(t *testing.T) {
	...
	for _, v := range variableMapSubstituteTests {
		...
		ctx := ContextForExecusion(context.Background(), v.Filter)
		_, err := vars.Substitute(ctx, v.Expr.(parser.VariableSubstitution))
		if err != nil {
			if len(v.Error) < 1 {
				t.Errorf("%s: unexpected error %q", v.Name, err)
			} else if err.Error() != v.Error {
