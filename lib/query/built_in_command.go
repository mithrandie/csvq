...
func LoadStatementsFromFile(ctx context.Context, tx *Transaction, fpath parser.Identifier) (statements []parser.Statement, err error) {
	...
	statements, _, err = parser.Parse(content, fpath.Literal, tx.Flags.DatetimeFormat, tx.Flags.GetTimeLocation(), false, tx.Flags.AnsiQuotes)
	if err != nil {
...
func ParseExecuteStatements(ctx context.Context, scope *ReferenceScope, expr parser.Execute) ([]parser.Statement, error) {
	...
	statements, _, err := parser.Parse(input, fmt.Sprintf("(L:%d C:%d) EXECUTE", expr.Line(), expr.Char()), scope.Tx.Flags.DatetimeFormat, scope.Tx.Flags.GetTimeLocation(), false, scope.Tx.Flags.AnsiQuotes)
	if err != nil {
		err = NewSyntaxError(err.(*parser.SyntaxError))
	}
	return statements, err
