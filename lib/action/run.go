...
func Run(ctx context.Context, proc *query.Processor, input string, sourceFile string, outfile string) error {
	...
	statements, _, err := parser.Parse(input, sourceFile, proc.Tx.Flags.DatetimeFormat, proc.Tx.Flags.GetTimeLocation(), false, proc.Tx.Flags.AnsiQuotes)
	if err != nil {
...
func LaunchInteractiveShell(ctx context.Context, proc *query.Processor) error {
	...
	for {
		...
		statements, _, e := parser.Parse(strings.Join(lines, "\n"), "", proc.Tx.Flags.DatetimeFormat, proc.Tx.Flags.GetTimeLocation(), false, proc.Tx.Flags.AnsiQuotes)
		if e != nil {
