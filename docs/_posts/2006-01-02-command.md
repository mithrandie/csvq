...
: Field delimiter for csv, or delimiter positions for Fixed-Length Format.
  The default is "," for csv files, "\t" for tsv files.
...
--json-query, -j JSON_QUERY
: [JSON_QUERY]({{ '/reference/json.html#query' | relative_url }}) for JSON data passed from standard input.
...
: Field delimiter for CSV, or delimiter positions for FIXED.
  Delimiter positions can be specified as a JSON Array of integers.
...
--pretty-print, -P
: Make JSON output easier to read
