...
If no _compare_ is match, then returns the _result_ of the ELSE expression or a null if there is no ELSE expression.
...
| Integer  | String   | If a string is a representation of a decimal integer, then it is converted to an integer. Otherwise it is converted to a null. |
